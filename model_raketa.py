...
class RocketSimulatorGUI:
    ...
    def setup_styles(self):
        ...
        if self.current_theme == 'dark':
            ...
            style.configure('TLabel', font=('Segoe UI', 10, 'normal'), background=bg_color, foreground=fg_color)
            style.configure('TButton', font=('Segoe UI', 10, 'normal'), background=button_bg, foreground=button_fg)
            style.map('TButton', background=[('active', accent_color), ('pressed', frame_bg)], foreground=[('active', button_fg), ('pressed', button_fg)])
            style.configure('TEntry', font=('Segoe UI', 10, 'normal'))
            style.configure('TLabelframe', background=frame_bg, borderwidth=2, relief='solid')
            style.configure('TLabelframe.Label', background=frame_bg, font=('Segoe UI', 11, 'bold'), foreground=fg_color)
            style.configure('TNotebook', background=bg_color, borderwidth=0)
            style.configure('TNotebook.Tab', background=button_bg, foreground=button_fg, padding=[12, 6], font=('Segoe UI', 10, 'normal'))
            style.map('TNotebook.Tab', background=[('selected', accent_color), ('active', frame_bg)], foreground=[('selected', button_fg), ('active', button_fg)])
